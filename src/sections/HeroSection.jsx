...
const HeroSection = () => {
  return (
    <section className="flex flex-col items-center min-h-screen justify-center bg-[url('/assets/heroBanner.png')] bg-cover bg-center bg-no-repeat px-4 pt-20">
        ...
        <Animated delay={0.2}>
            <p className='text-zinc-600 max-w-md text-center mt-3'>Experience carefully curated menus, fresh local incredients and impeccable service in a space made for every celebration.</p>
        </Animated>
        ...
        <Animated className="flex items-center justify-center md:justify-start mt-9">
            <div className="flex -space-x-3.5 pr-3">
                {avatars.map((src,index)=>{
                    <img  key={index} src={src} alt="guest" className="size-10 border-2 border-slate-50 rounded-full hover:-translate-y-px transition"></img>
                })}
            </div>
            <div>
                ...
                <p className="text-zinc-800">4.5 Rating - 10,000 reviews</p>
            </div>
        </Animated>
